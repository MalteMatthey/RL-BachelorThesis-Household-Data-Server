...
async def get_weather_obs(location_id: int = Query(...), start: datetime = Query(...), end: datetime = Query(...)):
    return await fetch_timeseries(db.weather_obs_tbl, db.weather_obs_tbl.c.datetime, {"location_id": location_id},
                                  start, end)


@router.get("/weather_fcst")
async def get_weather_forecast(location_id: int = Query(...), start: datetime = Query(...), end: datetime = Query(...)):
    return await fetch_timeseries(db.weather_fc_tbl, db.weather_fc_tbl.c.forecast_run, {"location_id": location_id},
                                  start, end)
